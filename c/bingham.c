 ...
 */
 ...
 // For visualizing, we need to know which data pt came from which distribution
 // Hence, append a column to X which has the component's index.
void bingham_mixture_sample(double **X2, bingham_mix_t *BM, int n)
{
  int i, j, ind;
  double arr[n];//maintain the index of BM from which the sample was drawn 
  int d = BM->B[0].d;
  double **X = new_matrix2(n, d);
  // double **X2 = new_matrix2(n, d+1);

  if (n == 1) {
    ind = pmfrand(BM->w, BM->n);
    bingham_sample(X, &BM->B[ind], 1);
    arr[0] = ind;
  }
  else if (n < 100) {
    for (i = 0; i < n; i++)
    ...
    int ntot = 0;
    for (i = 0; i < BM->n; i++) {
      ni[i] = round(n * BM->w[i]);//ni is no of samples from each component. Don't get confused by n(no of samples) and BM->n(no of comp)
      ntot += ni[i];// ntot is the actual no of samples right now, accomodating the errors generated by rounding off
    }
    if (ntot < n) {  // too few samples
      for (i = ntot; i < n; i++) {
  j = pmfrand(BM->w, BM->n); //take one more sample from jth component
  ni[j]++;
      }
    }
    else {
      while (ntot > n) {  // too many samples
  j = pmfrand(BM->w, BM->n); //take one less sample from jth component
  if (ni[j] > 0) {
    ni[j]--;
    ntot--;//this line is useless. And so wood be ntot++ in the previous loop, if it was there
  }
      }
    }
    ...
    for (i = 0; i < BM->n; i++) {   
      ...
      ntot += ni[i];
      arr[i] = i; //note this time we re looping over BM->n and not n. We are taking ni[i] samples. 
    }

    // other way is use analogy of add_rows_matrix2 in util.c
    int iter;
    for(iter = 0; iter<n; iter++){
      memcpy(X2[iter], X[iter], d*sizeof(double));
    }
  ...
  //debug end
    for(iter = 0; iter<n; iter++){
      memcpy(&X2[iter][d], &arr[iter], sizeof(double));
    }
    // free_matrix2(X);
