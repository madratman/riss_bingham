function plot_bingham_cluster_poster(bingham_cluster, no_of_samples, flag_plot_type)
% TODO weights as arguments. Sample properly!!
% In one circ-one circ regression problems aka parallel jaw regrasp
% problems, some binghams will be empty. Account for edge cases in all plot
% types. 
...
elseif strcmp(flag_plot_type, 'rotations')
    if size(bingham_cluster, 2) < 8;
        for i = 1:size(bingham_cluster, 2)
            i
            if(~isempty(bingham_cluster(i)))
                eval(['plot_bingham_rotations_poster(bingham_cluster(' num2str(i) '), no_of_samples, color_cell{' num2str(i) '});'])
            end
        end
    else
        for i = 1:size(bingham_cluster, 2)
            i
            if(~isempty(bingham_cluster(i)))
                eval(['plot_bingham_rotations_poster(bingham_cluster(' num2str(i) '), no_of_samples, rand(1,3));'])'
            end
        end
    end
