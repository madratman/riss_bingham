function plot_quaternions_cluster_poster(quat_cluster, flag_plot_type)
...
elseif strcmp(flag_plot_type, 'rotations')
    if length(quat_cluster) < 8;
        for i = 1:length(quat_cluster)
            i
            if(~isempty(cell2mat(quat_cluster(i))))
                eval(['plot_quaternions_rotations_poster(cell2mat(quat_cluster(' num2str(i) ''')), color_cell{' num2str(i) '});'])
            end
        end
    else
        for i = 1:length(quat_cluster)
            i
            if(~isempty(cell2mat(quat_cluster(i))))
                eval(['plot_quaternions_rotations_poster(cell2mat(quat_cluster(' num2str(i) ''')), rand(1,3));'])
            end
        end
    end
