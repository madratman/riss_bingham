...
for i = 1:6
   eval(['closest_quat_indices{i} = find_closest_quaternions(pred_face_' num2str(i) '_as_' num2str(i) '_init(1:100:end,:), face_' num2str(i)  '_init);'])
end
...
%   Only calculate errors for those points which were correctly classified.
    if(i==1|i==2)
        ...
        eval(['euler_actual = EulerAngles(quaternion(pred_face_' num2str(i) '_as_' num2str(i) '_final(1:100:end,:)), ''zyx'');'])
    elseif(i==3|i==4)
        eval(['euler_predicted = EulerAngles(quaternion(face_' num2str(i) '_final_predicted_without_vrep), ''xyz'');'])
        eval(['euler_actual = EulerAngles(quaternion(pred_face_' num2str(i) '_as_' num2str(i) '_final(1:100:end,:)), ''xyz'');'])
    elseif(i==5|i==6)
        eval(['euler_predicted = EulerAngles(quaternion(face_' num2str(i) '_final_predicted_without_vrep), ''xzx'');'])
        eval(['euler_actual = EulerAngles(quaternion(pred_face_' num2str(i) '_as_' num2str(i) '_final(1:100:end,:)), ''xzx'');'])
    end
