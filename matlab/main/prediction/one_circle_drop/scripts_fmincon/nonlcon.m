...
quat_trans = quat_trans.normalize; %normalize. WHERE?
quat_input = quaternion(quat_input);
% quat_input = quat_input.normalize, % No need
% quat_input = (quaternion, quat_input)
for i = 1:length(theta)
    final_quat = quaternion([cos(theta(i)/2) 0 0 sin(theta(i)/2)]); % No need to normalize final_quat
    % As the final_quat is in plane normal to Z axis.
    intermediate = times(quat_input(i), quat_trans) - final_quat;
    intermediate = intermediate.normalize;
    ceq(i,:) = from_quaternion_class_to_array(intermediate);
