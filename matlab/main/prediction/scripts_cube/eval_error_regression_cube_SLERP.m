...
%   Only calculate errors for those points which were correctly classified.
    if(i==1|i==2)
        ...
        eval(['euler_actual = EulerAngles(quaternion(pred_face_' num2str(i) '_as_' num2str(i) '_final_actual), ''zyx'');'])
    elseif(i==3|i==4)
        eval(['euler_predicted = EulerAngles(quaternion(face_' num2str(i) '_final_predicted_without_vrep), ''xyz'');'])
        eval(['euler_actual = EulerAngles(quaternion(pred_face_' num2str(i) '_as_' num2str(i) '_final_actual), ''xyz'');'])
    elseif(i==5|i==6)
        eval(['euler_predicted = EulerAngles(quaternion(face_' num2str(i) '_final_predicted_without_vrep), ''xzx'');'])
        eval(['euler_actual = EulerAngles(quaternion(pred_face_' num2str(i) '_as_' num2str(i) '_final_actual), ''xzx'');'])
    end
