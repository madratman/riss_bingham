...
%     eval(['test = isempty(pred_face_' num2str(i) '_as_' num
    eval(['test = isempty(pred_face_' num2str(i) '_as_' num2str(i) '_final);'])
    if(~test)
        if(i==1|i==2)
            eval(['euler_actual = EulerAngles(quaternion(pred_face_' num2str(i) '_as_' num2str(i) '_final), ''zyx'');'])
            eval(['training_target = EulerAngles(quaternion(face_' num2str(i) '_final), ''zyx'');'])
        elseif(i==3|i==4)
        ...
        elseif(i==5|i==6)
            ...
            eval(['training_target = EulerAngles(quaternion(face_' num2str(i) '_final), ''xzx'');'])
        end
    ...
    %   cos. And use same hyperparams for both GPs. 
        ...
        hyp = minimize(hyp, @gp, -1000, @infExact, [], covfunc, likfunc, x, y_cos)    
        [ymu_cos ys2 fmu fs2 lp] = gp(hyp, @infExact, meanfunc, covfunc, likfunc, x, y_cos, z);
        hyp.cov

        covfunc = @covQuaternionArc; sf = 0.08; lambda = 0.4; hyp.cov = [sf; lambda];
        hyp = minimize(hyp, @gp, -1000, @infExact, [], covfunc, likfunc, x, y_sin);
