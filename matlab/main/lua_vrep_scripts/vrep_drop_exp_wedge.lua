...
if (sim_call_type==sim_childscriptcall_initialization) then
    ...
    init_pose=io.open("/home/ratnesh/projects/riss/windows_riss_code/bingham/matlab/simple_solids/data/vrep_exp_wedge/init_quat.csv", "w");
    final_pose=io.open("/home/ratnesh/projects/riss/windows_riss_code/bingham/matlab/simple_solids/data/vrep_exp_wedge/final_quat.csv", "w");
    --init_pose_euler=io.open("/home/ratnesh/projects/riss/windows_riss_code/bingham/matlab/simple_solids/data/vrep_exp_wedge/init_euler.csv", "w");
    final_pose_euler=io.open("/home/ratnesh/projects/riss/windows_riss_code/bingham/matlab/simple_solids/data/vrep_exp_wedge/final_euler.csv", "w");
...
if (sim_call_type==sim_childscriptcall_actuation) then
    if count < threshold then 
        ...
        local orientation_final=simGetObjectOrientation(handle, -1)

        if (count%100==0) then
        
            if count ~= 0 then 
                -- no final pose written at the beginning of the simulation
                orientation_final[1] = orientation_final[1]*r2d;
                orientation_final[2] = orientation_final[2]*r2d;
                orientation_final[3] = orientation_final[3]*r2d;

                final_pose:write(count/100,",", quaternion_final[4], ","
                    , quaternion_final[1], ",", quaternion_final[2], ",", quaternion_final[3], "\n")
                final_pose_euler:write(count/100,",", orientation_final[1], ","
                    , orientation_final[2], ",", orientation_final[3], "\n")
